...
public class GangChatCmd(ICS2Gangs gangs): Command(gangs)
{
    public override void OnCommand(CCSPlayerController? executor, CommandInfo info)
    {
        ...
        var message = StringUtils.StripChatColors(info.ArgString);
        ...
        Task.Run(async () => {
            ...
            var gang = await gangs.GetGangsService().GetGang(senderPlayer.GangId.Value);
            if (gang == null)
            {
                Server.NextFrame(() => {
                    executor.PrintLocalizedChat(gangs.GetBase().Localizer, "command_error",
            ...
            if (!gang.Chat) {
                Server.NextFrame(() => {
                    executor.PrintLocalizedChat(gangs.GetBase().Localizer, "command_error",
                        "Your gang has not unlocked gang chat yet.");
                });
                return;
            }

            Server.NextFrame(() => {
                gangs.GetAnnouncerService().AnnounceToGangLocalized(gang, gangs.GetBase().Localizer, "gang_chat_format", gang.Name, senderPlayer.PlayerName ?? "Unknown", message);
            });
        });
