...
namespace plugin.commands.native;

public class NativeTargetModified
{
    ...
    public NativeTargetModified(string target)
    {
        Raw = target.Trim();
    ...
    private bool TargetPredicate(CCSPlayerController player, CCSPlayerController? caller)
    {
        switch (Type)
        {
            ...
            case TargetType.GroupDead:
                return player.PlayerPawn is
                    { IsValid: true, Value.LifeState: (byte)LifeState_t.LIFE_DEAD or (byte)LifeState_t.LIFE_DYING };
    ...
    public TargetResult GetTarget(CCSPlayerController? caller)
    {
        var players = Utilities.GetPlayers().Where(player =>
            ...
            TargetPredicate(player, caller)).ToList();
